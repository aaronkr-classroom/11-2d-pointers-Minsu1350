...
int main(void) {
	...
	printf("[1단계] 매장 포인터 할당 (2차원)\n");

	// 2차원 할당: 각 매장의 부서 포인터 할당
	for (int i = 0; i < stores; i++) {
		inventory[i] = (int**)malloc(depts * sizeof(int*));
	}
	printf("[2단계] 각 매장의 부서 할당 (2 * 3 = 6 포인터)\n");
	...
	printf("[3단계] 각 부서의 항목 할당 (2 * 3 * 4 = 24 포인터)\n");
	...
	int sample_prices[2][3][4] = {
		// 매장 1: Emart
		{
			{299, 149, 79, 199}, // 전자제품 (TV, Headphones, USB, Monitor)
			{39, 59, 29, 49}, // 의류 (Pants, Shirt, Socks, Hat)
			{89, 129, 49, 59} // 가정용품 (...)
		},
		// 매장 2: Costco
		{
			{329, 159, 89, 179}, // 전자제품 (TV, Headphones, USB, Monitor)
